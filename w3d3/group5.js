...
function max_servings(recipe, available) {
    let totalServings = Number.MAX_VALUE;
    ...
    for (let ingredient in recipe) {
        ...
        let availableServings = Math.floor(available[ingredient]/recipe[ingredient]);
        if (availableServings < totalServings) {
            totalServings = availableServings;
        }
    }

    if (totalServings === Number.MAX_VALUE) {
        return 0;
    }
